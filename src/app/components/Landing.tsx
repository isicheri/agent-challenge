...
export default function CoplanaLanding() {
  return (
    <div className="min-h-screen dotted bg-white bg-gradient-to-b from-white to-gray-100 text-blk flex flex-col">
      {/* Hero Section */}
      ...
      <div className="header sticky top-0 z-[20] shadow-2xl shadow-black/60 bg-blk p-4 md:px-8 lg:px-16 flex items-center gap-4 justify-between">
       ...
       <Link href="/onboarding">
        <div className="login  p-3 text-white px-5 hover:underline ">CONTINUE FOR FREE  {" "} &rarr;</div>
       </Link>
      </div>

      <header className="flex flex-col px-6 md:px-8 lg:px-16 py-24">
      <div className="gradText w-max mb-4 font-semibold">BIG TOPICS, LITTLE TIMEFRAME.</div>
        <motion.h1
      ...
      <section className="px-6 py-16 bg-white">
        ...
        <div className="grid md:grid-cols-3 gap-4 max-w-6xl mx-auto md:hover:*:not-hover:blur-lg md:hover:*:not-hover:scale-90 ">
          {[
            ...
            <motion.div
              key={index}
              style={{overflow:"hidden", backgroundImage: `url("/grid.svg")`, backgroundSize: 200}} 
            ...
            >
             ...
             <Image src={`/${feature.icon}.svg`} width={45} height={45} alt={feature.icon} className="relative" />
              <h3 className="text-3xl relative pt-26  font-semibold mb-2">{feature.title}</h3>
      ...
      <div className="w-full text-center flex justify-center"> <div className="gradText w-max">COPLANA BETA</div></div>
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4">
          And that's just the beginning...
        </h2>
        <p className="max-w-2xl mx-auto text-gry text-lg mb-8">
          Soon, Coplana will do more than plan. It'll test your readiness with
          auto-generated quizzes and insights, helping you identify your strong
          and weak areas before the big day.
        </p>
        <motion.button
      ...
      <footer className="mt-auto bg-white border-t border-gray-200 py-6 text-center text-sm text-gray-500">
        © {new Date().getFullYear()} Coplana. Crafted to help YOU study smarter.
      </footer>
    </div>
  );
}
